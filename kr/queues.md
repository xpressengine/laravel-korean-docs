...
때때로 여러분은 큐 작업을 지연시키기를 원할 수 있습니다. 예를들어 고객이 가입후 15분 후에 이메일을 보내도록 큐 작업을 지연시키길 원할 수 있습니다. `Queue::later` 메소드를 사용하면 이와 같은 작업을 수행할 수 있습니다:
...
데몬 큐  workers는 각각의 작업이 진행되기 전에 프레임워크를 재시작하지 않습니다. 따라서 많은 자원을 사용하는 작업을 사용하기 전에 주의해야 합니다. 예를들어 GD 라이브러리로 이미지 처리를 하고 있다면 작업이 종료된 후 `imagedestroy`로 메모리를 비워주어야 합니다.
